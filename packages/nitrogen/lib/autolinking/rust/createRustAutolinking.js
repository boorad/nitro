 ...
 */
function findMatchingClose(str, startIdx, open, close) {
    let depth = 0;
 ...
 */
function extractTraitMethods(rustContent, traitName) {
    // Find the trait block
    const traitStart = rustContent.indexOf(`pub trait ${traitName}: Send + Sync {`);
    if (traitStart === -1)
        return [];
 ...
 */
export function createRustFactory(allFiles) {
    ...
    for (const hybridObjectName of Object.keys(autolinkedHybridObjects)) {
        ...
        const traitFile = allFiles.find((f) => f.name === `${HybridTSpec}.rs` && f.language === "rust");
        if (traitFile != null) {
            ...
            implBlocks.push(`/// Wrapper that delegates the generated trait to the user's implementation.
