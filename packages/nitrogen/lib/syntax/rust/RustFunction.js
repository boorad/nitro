 ...
 */
export function createRustFunction(funcType) {
    ...
    const rustReturnSuffix = rustReturnType === "()" ? "" : ` -> ${rustReturnType}`;
    ...
    if (returnBridged.needsSpecialHandling) {
        const resultConversion = returnBridged.parseFromCppToRust("__result", "rust");
    ...
    const importsBlock = rustImports.length > 0 ? rustImports.join("\n") + "\n" : "";
    const code = `
