import { createFileMetadataString, createRustFileMetadataString, isNotDuplicate, toSnakeCase, } from "../helpers.js";
 ...
 */
export function createRustHybridObject(spec) {
    const files = [];
...
function createRustTraitAndFfi(spec) {
...
pub unsafe extern "C" fn ${name.HybridTSpec}_set_${rustPropName}(ptr: *mut std::ffi::c_void, value: ${ffiReturnType}) {
    ...
    for (const method of spec.methods) {
        ...
        const effectiveReturnType = method.returnType.kind === "promise"
        ...
        const ffiParamsWithComma = ffiParams.length > 0 ? `, ${ffiParams.join(", ")}` : "";
        // Build conversion code for parameters and return value
        const paramConversions = [];
        const traitCallArgs = [];
        ...
        if (paramConversions.length === 0 && !returnBridged.needsSpecialHandling) {
            ...
            const lines = [];
            lines.push(`        let obj = &mut *(ptr as *mut Box<dyn ${name.HybridTSpec}>);`);
            for (const conv of paramConversions) {
                lines.push(`        ${conv}`);
            }
            if (returnBridged.needsSpecialHandling) {
                lines.push(`        let __result = ${traitCall};`);
                lines.push(`        ${returnBridged.parseFromRustToCpp("__result", "rust")}`);
            }
            else {
...
pub unsafe extern "C" fn __nitrogen_free_cstring(ptr: *mut std::ffi::c_char) {
  ...
  `.trim());
    ...
    const importsBlock = rustImports.length > 0 ? "\n" + rustImports.join("\n") + "\n" : "";
...
function createCppRustBridgeHeader(spec) {
    const name = getHybridObjectName(spec.name);
    ...
    for (const prop of spec.properties) {
        ...
        externDecls.push(`${ffiType} ${name.HybridTSpec}_get_${rustPropName}(void* rustPtr);`);
        if (!prop.isReadonly) {
            externDecls.push(`void ${name.HybridTSpec}_set_${rustPropName}(void* rustPtr, ${ffiType} value);`);
        }
    }
    for (const method of spec.methods) {
        const rustMethodName = toSnakeCase(method.name);
        ...
        externDecls.push(`${ffiReturnType} ${name.HybridTSpec}_${rustMethodName}(void* rustPtr${paramsWithComma});`);
    }
    ...
    for (const prop of spec.properties) {
        ...
        if (bridged.needsSpecialHandling) {
            ...
            propertyImpls.push(`inline ${cppType} ${getterName}() override { return ${converted}; }`);
        }
        else {
            propertyImpls.push(`inline ${cppType} ${getterName}() override { return ${name.HybridTSpec}_get_${rustPropName}(_rustPtr); }`);
        }
        if (!prop.isReadonly) {
            ...
            if (bridged.needsSpecialHandling) {
                ...
                propertyImpls.push(`inline void ${setterName}(${paramType} ${prop.name}) override { ${name.HybridTSpec}_set_${rustPropName}(_rustPtr, ${converted}); }`);
            }
            else {
                propertyImpls.push(`inline void ${setterName}(${paramType} ${prop.name}) override { ${name.HybridTSpec}_set_${rustPropName}(_rustPtr, ${prop.name}); }`);
            }
        }
    }
    ...
    for (const method of spec.methods) {
        ...
        const ffiArgsWithComma = ffiArgs.length > 0 ? `, ${ffiArgs.join(", ")}` : "";
        ...
        if (isPromise) {
            ...
            if (hasReturn) {
                if (returnBridged.needsSpecialHandling) {
                    ...
                    methodImpls.push(`inline ${returnType} ${method.name}(${params}) override {\n` +
            ...
            const converted = returnBridged.parseFromRustToCpp("__result", "c++");
            methodImpls.push(`inline ${returnType} ${method.name}(${params}) override { auto __result = ${ffiCall}; return ${converted}; }`);
        }
        else {
            methodImpls.push(`inline ${returnType} ${method.name}(${params}) override { ${hasReturn ? "return " : ""}${ffiCall}; }`);
        }
    }
...
function createCppRustBridgeImpl(spec) {
    const name = getHybridObjectName(spec.name);
    const cxxNamespace = spec.config.getCxxNamespace("c++");
    const implCode = `
