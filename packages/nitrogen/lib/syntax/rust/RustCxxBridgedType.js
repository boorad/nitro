 ...
 */
export class RustCxxBridgedType {
    type;
    constructor(type) {
        this.type = type;
    }
    ...
    getExtraFiles() {
        ...
        switch (this.type.kind) {
            ...
            case "struct": {
                ...
                files.push(createRustStruct(structType.structName, structType.properties));
                break;
            }
            case "variant": {
     ...
     */
    parseFromCppToRust(parameterName, inLanguage) {
        switch (this.type.kind) {
            case "string":
                switch (inLanguage) {
                    case "rust":
            ...
            case "function": {
                ...
                switch (inLanguage) {
                    case "rust": {
                        ...
                        const rustParams = funcType.parameters.map((p, i) => `__p${i}: ${p.getCode("rust")}`);
                        const callArgs = funcType.parameters.map((_p, i) => `__p${i}`);
                        const rustReturnType = funcType.returnType.getCode("rust");
                        const returnSuffix = rustReturnType === "()" ? "" : ` -> ${rustReturnType}`;
                        return (`{ let __wrapper = Box::from_raw(${parameterName} as *mut ${funcStructPath}); ` +
                    ...
                    case "c++": {
                        ...
                        const returnArrow = ffiReturnType === "void" ? "" : `-> ${ffiReturnType} `;
                        return (`[&]() -> void* { ` +
     ...
     */
    parseFromRustToCpp(parameterName, inLanguage) {
        switch (this.type.kind) {
            case "string":
                switch (inLanguage) {
                    case "rust":
